...
- **Navegação do CRM (Funil) no Mobile:** Adicionada a opção de acesso ao **CRM (Funil)** na barra inferior móvel (`BottomNav`), tornando o funil 100% acessível em smartphones.
- **Funil CRM Responsivo com Abas Adaptativas (Segmented Control):** No celular, o CRM substitui a rolagem horizontal rígida por um seletor de abas deslizante `[ Contato | Triagem | 1ª Sessão | Ativo ]` com botões rápidos de transição entre etapas (`<ChevronRight />`), mantendo a visão Kanban em 4 colunas no Desktop.
- **Remoção de Barras de Rolagem Nativas em Modais:** Aplicada a classe `hide-scrollbar` com máscara de gradiente esmaecido (*scroll mask*) no modal de *Novo Paciente* e de *Detalhes do Agendamento*, garantindo uma rolagem interna fluida sem poluição de barras visuais.
- **Integração Visual de Menus (Sidebar & BottomNav):** A barra lateral (`Sidebar`) e a navegação inferior móvel (`BottomNav`) adotam `bg-[var(--bg)]`, fundindo-se 100% à cor do canvas nos dois temas (Creme Orgânico `#FAF8F4` no Claro e Navy Profundo `#101722` no Escuro).
- **Elevação de Superfícies em 3 Níveis no Modo Claro:** Implementada a hierarquia visual tátil com Fundo (`#FAF8F4`), Cards/Modais (`#FFFFFF`) e Quadros Internos (`#F5F2EA`), proporcionando contraste elegante e reduzindo a fadiga visual.
...
- **Rodapés e Rótulos Uniformes:** Botão secundário de modais padronizado como **"Cancelar"** (`triggerClose`), e cabeçalhos de observações clínica com ícone `<Sparkles />` no tom da ação e badge de estado **"Preenchido"** dinâmico.
