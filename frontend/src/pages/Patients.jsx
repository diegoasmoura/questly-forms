...
export default function Patients() {
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      ...
      parsed.forEach((p, i) => {
        ...
        else if (p.cpf.replace(/\D/g, "").length !== 11)
          errors.push(" Linha " + n + ": CPF invalido");
        if (!p.birthDate) errors.push(" Linha " + n + ": Data obrigatoria");
        else if (!/^\d{4}-\d{2}-\d{2}$/.test(p.birthDate))
          errors.push(" Linha " + n + ": Data invalida (use DD-MM-YYYY)");
        if (!p.email) errors.push(" Linha " + n + ": E-mail obrigatorio");
        else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(p.email))
          errors.push(" Linha " + n + ": E-mail invalido");
    ...
    for (const p of importedPatients) {
      try {
        ...
        setImportResults((prev) => [
          ...prev,
          "Erro: " + p.name + " (" + (err.message || "ja cadastrado") + ")",
        ]);
      }
      setImportProgress(
        Math.round(((success + importResults.length) / total) * 100)
      );
    }
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        !event.target.closest(".registration-dropdown")
      ) {
        setShowRegistrationDropdown(false);
      }
      if (
        showEmptyRegistrationDropdown &&
        !event.target.closest(".empty-registration-dropdown")
      ) {
  ...
  const handleAddPatient = async (e) => {
    ...
    if (!newPatient.name || !newPatient.cpf || !newPatient.birthDate) {
      ...
      setErrorMessage(
        "Por favor, preencha todos os campos obrigatórios na aba Identificação."
      );
      return;
    }
    ...
    if (
      ...
      setErrorMessage(
        "Por favor, preencha todos os campos obrigatórios na aba Contato."
      );
      return;
    }
  ...
  const handleDeletePatient = async (id) => {
    if (
      !confirm(
        "Tem certeza que deseja excluir este paciente? Todo o histórico será perdido."
      )
    )
      return;
    try {
  ...
  return (
    <div className="p-4 sm:p-6 h-full flex flex-col overflow-hidden animate-fade-in">
      {/* ── Header ── */}
      <div className="flex items-center justify-between gap-3 mb-4 sm:mb-6 shrink-0">
        <div>
          <h1 className="text-xl sm:text-2xl font-semibold text-slate-800">Pacientes</h1>
          {/* Subtítulo oculto em mobile — desperdiça espaço valioso */}
          <p className="hidden sm:block text-sm text-slate-500">
            Gerencie seus pacientes e seus históricos clínicos.
          </p>
        </div>
        ...
        <div className="relative registration-dropdown">
          <button
            ...
            className="btn btn-primary px-3 sm:px-4 flex items-center gap-1.5 sm:gap-2"
          >
            <Plus size={18} />
            <span>Cadastrar</span>
            <ChevronDown
              ...
              className={`transition-transform duration-200 ${
                showRegistrationDropdown ? "rotate-180" : ""
              }`}
            />
          </button>
      ...
      {/* ── Search & Filters ── */}
      <div className="flex items-center gap-2 sm:gap-4 mb-4 sm:mb-6 shrink-0">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            /* Placeholder curto em mobile, completo em sm+ */
            placeholder="Buscar..."
        ...
        <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-lg border border-slate-200 shrink-0">
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-md transition-all ${
              viewMode === "grid"
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="card p-10 sm:p-20 text-center border-dashed border-2 border-slate-200">
            <div className="w-16 h-16 sm:w-20 sm:h-20 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6">
              <Users size={32} className="text-slate-400 sm:size-40" />
            </div>
            <h3 className="text-lg sm:text-xl font-semibold text-slate-700 mb-2">
              {searchQuery ? "Nenhum paciente encontrado" : "Nenhum paciente cadastrado"}
            </h3>
            <p className="text-slate-500 mb-6 sm:mb-8 max-w-sm mx-auto text-sm">
              {searchQuery
                ? "Tente um termo de busca diferente"
                : "Comece cadastrando seu primeiro paciente para acompanhar sua evolução clínica."}
            </p>
            {!searchQuery && (
              <div className="relative empty-registration-dropdown inline-block">
                <button
                  onClick={() =>
                    setShowEmptyRegistrationDropdown(!showEmptyRegistrationDropdown)
                  }
                ...
                >
                  ...
                  <ChevronDown
                    size={14}
                    className={`transition-transform duration-200 ${
                      showEmptyRegistrationDropdown ? "rotate-180" : ""
                    }`}
                  />
                </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 pt-2 sm:pt-4 pb-10 overflow-visible">
          ...
          <div className="space-y-3 pt-4 pb-10 overflow-visible">
            {filteredPatients.map((patient) => (
              <PatientListRow
                key={patient.id}
                patient={patient}
      ...
      {showImportModal && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-slate-900/30 backdrop-blur-sm">
          <div className="card w-full sm:max-w-2xl animate-scale-in max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col rounded-b-none sm:rounded-2xl">
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    ...
                    {importStep === "preview" &&
                      importedPatients.length + " paciente(s) encontrado(s)"}
                ...
                <>
                  <div className="p-4 sm:p-5 bg-slate-50 rounded-2xl border border-slate-200">
                    <div className="flex items-start gap-4">
                      <div className="w-9 h-9 rounded-xl bg-slate-900 text-emerald-400 flex items-center justify-center font-bold shrink-0">
                        1
                      </div>
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-slate-500 mb-3">
                          Use a planilha padrão.
                        </p>
                        <button
                          onClick={generateExcelTemplate}
                          className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-700 hover:bg-slate-50"
                        >
                  ...
                  <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 flex items-start gap-2">
                    ...
                    <p className="text-xs text-blue-700">
                      Campos obrigatórios: Nome, CPF, Data, E-mail, Telefone, Nome e
                      Telefone Emergência.
                    </p>
                  </div>
                </>
              ...
              {importStep === "empty" && (
                <div className="py-16 flex flex-col items-center gap-4 text-center">
                  ...
                  <p className="text-xs text-slate-500">
                    Verifique se a aba "Pacientes" tem dados
                  </p>
                  <button
                    ...
                    <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-xs font-bold text-red-700">
                          {importErrors.length} problema(s)
                        </p>
                      </div>
                  ...
                  {importResults.length > 0 && (
                    <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                      <div className="flex items-center gap-2 mb-2">
                        <AlertTriangle size={14} className="text-red-500" />
                        <p className="text-xs font-bold text-red-700">
                          {importResults.length} problema(s)
                        </p>
                      </div>
                      <ul className="space-y-1 max-h-24 overflow-y-auto">
                  ...
                  <div className="rounded-xl border border-slate-200 overflow-hidden">
                    <div className="bg-slate-50 px-4 py-2 border-b border-slate-200 flex items-center justify-between">
                      <p className="text-xs font-bold text-slate-600">
                        Prévia — {importedPatients.length}
                      </p>
                      {importErrors.length === 0 && importResults.length === 0 && (
                    ...
                    <div className="overflow-x-auto max-h-56">
                      <table className="w-full text-xs">
                        <thead className="bg-slate-50 sticky top-0">
                          <tr>
                            {["Nome", "CPF", "E-mail", "Telefone"].map((h) => (
                              <th
                                key={h}
                                className="text-left px-3 py-2 text-slate-500 font-bold"
                              >
                                {h}
                              </th>
                            ))}
            ...
            <div className="p-4 sm:p-6 border-t border-slate-200 bg-slate-50 shrink-0">
              {importStep === "idle" && (
                <button onClick={resetImportModal} className="btn btn-secondary w-full">
                  Fechar
                </button>
              )}
              {importStep === "preview" && (
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-slate-900/30 backdrop-blur-sm">
          <div className="card w-full sm:max-w-2xl animate-scale-in max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col rounded-b-none sm:rounded-2xl">
            ...
            <div className="p-4 sm:p-6 border-b border-slate-200 shrink-0">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Preencha os dados para o prontuário
                  </p>
                </div>
                <button
              ...
              <div className="flex gap-1 bg-slate-100 p-1 rounded-lg overflow-x-auto">
                {[
                  ...
                  >
                    <tab.icon size={14} aria-label={tab.label} />
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6">
              <form id="patient-form" onSubmit={handleAddPatient} className="space-y-4 sm:space-y-5">
                ...
                {addFormTab === "identity" && (
                  <div className="space-y-4 sm:space-y-5">
                    ...
                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            setNewPatient({ ...newPatient, isActive: !newPatient.isActive })
                          }
                    ...
                    {/* CPF + Nascimento — 2 colunas em mobile, sem quebrar */}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          value={newPatient.rg}
                          onChange={(e) =>
                            setNewPatient({ ...newPatient, rg: e.target.value })
                          }
                ...
                {addFormTab === "address" && (
                  <div className="space-y-4 sm:space-y-5">
                    {/* CEP full width + endereço abaixo em mobile */}
                    <div>
                      <label className="block text-xs font-semibold text-slate-600 mb-2">
                        CEP
                      </label>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setNewPatient({ ...newPatient, complement: e.target.value })
                          }
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setNewPatient({ ...newPatient, neighborhood: e.target.value })
                        }
                        placeholder="Bairro"
                      />
                    </div>
                  ...
                  <div className="space-y-4 sm:space-y-5">
                    ...
                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <label
                          className={`flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 rounded-lg text-xs font-medium cursor-pointer transition-all ${
                            uploading
                              ? "opacity-50"
      ...
      {/* ── Success Toast ── */}
      {successMessage && (
        <div className="fixed bottom-6 left-4 right-4 sm:left-auto sm:right-8 sm:w-auto z-[100] animate-slide-up">
          <div className="bg-emerald-600 text-white px-5 py-3 sm:px-6 sm:py-4 rounded-2xl shadow-2xl flex items-center gap-3 border border-emerald-500/50 backdrop-blur-sm">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center sm:p-4 bg-slate-900/30 backdrop-blur-sm">
      <div className="card w-full sm:max-w-2xl animate-scale-in max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col rounded-b-none sm:rounded-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6">
          <form id="edit-patient-form" onSubmit={handleSave} className="space-y-4 sm:space-y-5">
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500">
                    ...
                    {!formData.isActive && patient.inactivatedAt && (
                      <div className="flex items-center gap-1">
                        ...
                        <span>
                          Inatividade:{" "}
                          {new Date(patient.inactivatedAt).toLocaleDateString("pt-BR")}
                        </span>
                      </div>
                    )}
                  </div>
        ...
        {cleanupModal.open && (
          <div
            className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center z-[110]"
            onClick={() => setCleanupModal({ ...cleanupModal, open: false })}
          >
            <div
              className="bg-white rounded-3xl p-8 w-full max-w-sm mx-4 shadow-2xl animate-scale-in border border-slate-100"
...
function PatientCard({ patient, onDelete, onEdit }) {
  ...
  const { days: daysUntilBirthday, isWeek: isBirthdayWeek } =
    calculateDaysUntilBirthday(patient.birthDate);
  const attendance = patient.attendanceStats || {
    presente: 0,
    falta: 0,
    justificada: 0,
  };
  ...
  return (
    <div
      className={`card group hover:shadow-xl transition-all duration-300 flex flex-col h-full card-bone border-slate-100 relative ${
        !isActive ? "opacity-70" : ""
      } ${
        isBirthdayWeek
          ? "ring-2 ring-amber-400 border-amber-200 bg-gradient-to-br from-white to-amber-50/30 shadow-lg shadow-amber-100/50"
          : "hover:border-emerald-200"
      }`}
    >
      {isBirthdayWeek && (
        <div className="absolute -top-2 -right-2 w-8 h-8 sm:w-10 sm:h-10 bg-white rounded-xl sm:rounded-2xl shadow-xl flex items-center justify-center border border-amber-100 animate-bounce z-10">
          <PartyPopper size={16} className="text-amber-500" />
        </div>
      )}

      {/* Top Bar */}
      <div className="p-4 sm:p-5 pb-0 flex items-start justify-between relative z-0">
        <div className="flex items-center gap-3">
          <div
            className={`w-11 h-11 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl flex items-center justify-center font-bold text-lg sm:text-xl transition-all duration-300 ${
              isActive
                ? isBirthdayWeek
                  ? "bg-amber-500 text-white shadow-lg shadow-amber-200"
                  : "bg-slate-900 text-emerald-400 group-hover:bg-emerald-600 group-hover:text-white"
          ...
          <div className="flex flex-col gap-1">
            <h3
              className={`text-base sm:text-lg font-bold leading-tight transition-colors line-clamp-1 ${
                isBirthdayWeek
                  ? "text-amber-900"
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {daysUntilBirthday !== null && (
                <div className="relative group/tooltip">
                  <span
                    className={`flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider border cursor-pointer ${
                      isBirthdayWeek
                  ...
                  >
                    ...
                    {typeof daysUntilBirthday === "string"
                      ? "Aniversário!"
                      : `${daysUntilBirthday}d`}
                  </span>
      ...
      <div className="px-4 sm:px-5 py-4 sm:py-6">
        <div className="rounded-2xl border border-slate-100 bg-slate-50/50 overflow-hidden">
          <div className="grid grid-cols-2">
            {/* Sessões */}
            <div className="p-3 sm:p-4 border-r border-slate-100">
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-1">
                <Calendar size={11} className="text-emerald-500" /> Sessões
              </p>
              <div className="flex items-end gap-3">
                ...
                <div className="flex flex-col">
                  <span className="text-xl sm:text-2xl font-black text-slate-400 leading-none">
                    {attendance.falta}
                  </span>
                  <span className="text-[9px] font-bold text-rose-400 uppercase mt-1">
                    Faltas
                  </span>
                </div>
              </div>
            ...
            <div className="p-3 sm:p-4">
              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-1">
                <FileText size={11} className="text-emerald-500" /> Instrumentos
              </p>
              <div className="flex items-end gap-3">
                <div className="flex flex-col">
                  <span className="text-xl sm:text-2xl font-black text-slate-900 leading-none">
      ...
      <div className="px-4 sm:px-5 pb-4 sm:pb-5 mt-auto">
        ...
        {/* Botões com altura mínima de 44px para toque */}
        <div className="flex gap-2">
          <Link
            to={`/patients/${patient.id}`}
            className="flex-1 btn bg-slate-900 text-white hover:bg-slate-800 text-xs py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg shadow-slate-200 min-h-[44px]"
          >
            <FileText size={14} /> Prontuário
          </Link>
          <button
            onClick={() => onEdit(patient)}
            className="btn bg-white border border-slate-200 text-slate-400 hover:text-emerald-600 hover:border-emerald-200 hover:bg-emerald-50 text-xs px-3 py-3 rounded-xl font-bold flex items-center justify-center gap-1.5 transition-all min-h-[44px]"
          >
            <Pencil size={14} />
          ...
          <button
            ...
            className="btn bg-white border border-slate-200 text-slate-400 hover:text-rose-500 hover:border-rose-200 hover:bg-rose-50 text-xs px-3 py-3 rounded-xl font-bold flex items-center justify-center transition-all min-h-[44px]"
          >
            <Trash2 size={14} />
          </button>
        </div>
...
function PatientListRow({ patient, onDelete, onEdit }) {
  ...
  return (
    <div
      className={`card p-3 sm:p-4 flex items-center gap-3 sm:gap-5 transition-all relative overflow-hidden ${
        !isActive ? "opacity-70" : ""
      } ${
        isBirthdayWeek
    ...
    >
      <Link
        to={`/patients/${patient.id}`}
        className={`w-11 h-11 rounded-xl flex items-center justify-center font-bold text-base shrink-0 transition-all ${
          isActive
            ? isBirthdayWeek
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1 flex-wrap">
          <Link to={`/patients/${patient.id}`} className="group/name">
            <h4
              className={`font-bold transition-colors line-clamp-1 text-sm sm:text-base ${
                isBirthdayWeek
                  ? "text-amber-900"
          ...
          {daysUntilBirthday !== null && (
            <div className="relative group/tooltip">
              <span
                className={`px-1.5 py-0.5 rounded text-[8px] font-black uppercase border flex items-center gap-1 cursor-pointer ${
                  isBirthdayWeek
                    ? "bg-amber-500 text-white border-amber-400 animate-pulse"
              ...
              >
                <CakeSlice size={10} className={isBirthdayWeek ? "text-white" : "text-slate-400"} />
                {typeof daysUntilBirthday === "string"
                  ? daysUntilBirthday === "Hoje! 🎂"
                    ? "Niver Hoje!"
                    : daysUntilBirthday
                  : `Em ${daysUntilBirthday}d`}
              </span>
        ...
        {/* Em mobile mostra só o telefone para economizar espaço */}
      ...
      {/* Stats — oculto em mobile, visível em sm+ */}
      <div className="hidden sm:flex items-center gap-10 px-8 border-x border-slate-100">
        <div className="flex flex-col">
          <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-1">
          ...
          <div className="flex gap-4">
            ...
            <div className="flex flex-col">
              <span className="text-lg font-black text-slate-400 leading-none">
                {attendance.falta}
              </span>
              <span className="text-[9px] font-bold text-rose-400 uppercase mt-1">
                Faltas
      ...
      {/* Ações — altura mínima 44px para toque */}
      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
        <Link
          to={`/patients/${patient.id}`}
          className="p-2.5 rounded-xl bg-slate-900 text-white hover:bg-slate-800 transition-all shadow-md shadow-slate-200 min-h-[44px] min-w-[44px] flex items-center justify-center"
        >
          <FileText size={17} />
        </Link>
        <button
          onClick={() => onEdit(patient)}
          className="p-2.5 rounded-xl border border-slate-200 text-slate-400 hover:text-emerald-600 hover:border-emerald-200 hover:bg-emerald-50 transition-all min-h-[44px] min-w-[44px] flex items-center justify-center"
        >
          <Pencil size={17} />
        </button>
        <button
          onClick={() => onDelete(patient.id)}
          className="p-2.5 rounded-xl border border-slate-200 text-slate-400 hover:text-rose-500 hover:border-rose-200 hover:bg-rose-50 transition-all min-h-[44px] min-w-[44px] flex items-center justify-center"
        >
          <Trash2 size={17} />
        </button>
      </div>
